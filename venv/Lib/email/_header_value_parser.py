...
class DisplayName(Phrase):
    ...
    def value(self):
        ...
        if len(self) != 0 and quote:
            pre = post = ""
            if (
                self[0].token_type == "cfws"
                or isinstance(self[0], TokenList)
                and self[0][0].token_type == "cfws"
            ):
                pre = " "
            if (
                self[-1].token_type == "cfws"
                or isinstance(self[-1], TokenList)
                and self[-1][-1].token_type == "cfws"
            ):
                post = " "
...
_non_atom_end_matcher = re.compile(
    r"[^{}]+".format(re.escape("".join(ATOM_ENDS)))
).match
_non_printable_finder = re.compile(r"[\x00-\x20\x7F]").findall
_non_token_end_matcher = re.compile(
    r"[^{}]+".format(re.escape("".join(TOKEN_ENDS)))
).match
_non_attribute_end_matcher = re.compile(
    r"[^{}]+".format(re.escape("".join(ATTRIBUTE_ENDS)))
).match
_non_extended_attribute_end_matcher = re.compile(
    r"[^{}]+".format(re.escape("".join(EXTENDED_ATTRIBUTE_ENDS)))
).match
...
def _validate_xtext(xtext):
    ...
    if utils._has_surrogates(xtext):
        xtext.defects.append(
            errors.UndecodableBytesDefect("Non-ASCII characters found in header token")
        )
...
def get_encoded_word(value):
    ...
    if (
        len(remstr) > 1
        and remstr[0] in hexdigits
        and remstr[1] in hexdigits
        and tok.count("?") < 2
    ):
    ...
    if value and value[0] not in WSP:
        ew.defects.append(
            errors.InvalidHeaderDefect("missing trailing whitespace after encoded-word")
        )
    return ew, value
...
def get_phrase(value):
    ...
    try:
        ...
        phrase.defects.append(
            errors.InvalidHeaderDefect("phrase does not start with word")
        )
        ...
        if value[0] == ".":
            ...
            try:
                ...
                if value[0] in CFWS_LEADER:
                    token, value = get_cfws(value)
                    phrase.defects.append(
                        errors.ObsoleteHeaderDefect("comment found without atom")
                    )
                else:
                    raise
            phrase.append(token)
...
def get_local_part(value):
    ...
    try:
        ...
        local_part.defects.append(
            errors.NonASCIILocalPartDefect("local-part contains non-ASCII characters)")
        )
...
def get_obs_local_part(value):
        ...
        if value[0] == ".":
            if last_non_ws_was_dot:
                obs_local_part.defects.append(
                    errors.InvalidHeaderDefect("invalid repeated '.'")
                )
    ...
    if (
        ...
        obs_local_part.defects.append(
            errors.InvalidHeaderDefect("Invalid trailing '.' in local part")
        )
...
def get_dtext(value):
    ...
    ptext = ValueTerminal(ptext, "ptext")
    if had_qp:
        ptext.defects.append(
            errors.ObsoleteHeaderDefect("quoted printable found in domain-literal")
        )
    _validate_xtext(ptext)
    return ptext, value
...
def get_domain(value):
    ...
    domain.append(token)
    if value and value[0] == ".":
        domain.defects.append(
            errors.ObsoleteHeaderDefect("domain is not a dot-atom (contains CFWS)")
        )
...
def get_addr_spec(value):
    ...
    if not value or value[0] != "@":
        addr_spec.defects.append(
            errors.InvalidHeaderDefect("addr-spec local part with no domain")
        )
...
def get_angle_addr(value):
    ...
    if value and value[0] == ">":
        ...
        angle_addr.defects.append(
            errors.InvalidHeaderDefect("null addr-spec in angle-addr")
        )
        value = value[1:]
        ...
        try:
            ...
            if value[0] in CFWS_LEADER:
                ...
                if not value or value[0] in ",;":
                    ...
                    mailbox_list.defects.append(
                        errors.ObsoleteHeaderDefect("empty element in mailbox-list")
                    )
                else:
                    ...
                    mailbox_list.append(token)
                    mailbox_list.defects.append(
                        errors.InvalidHeaderDefect("invalid mailbox in mailbox-list")
                    )
            elif value[0] == ",":
                mailbox_list.defects.append(
                    errors.ObsoleteHeaderDefect("empty element in mailbox-list")
...
def get_group_list(value):
    ...
    if not value:
        group_list.defects.append(
            errors.InvalidHeaderDefect("end of header before group-list")
        )
    ...
    if value and value[0] in CFWS_LEADER:
        ...
        if not value:
            ...
            group_list.defects.append(
                errors.InvalidHeaderDefect("end of header in group-list")
            )
    ...
    if len(token.all_mailboxes) == 0:
        ...
        group_list.defects.append(
            errors.ObsoleteHeaderDefect("group-list with empty entries")
        )
        return group_list, value
    if leader is not None:
        token[:0] = [leader]
        ...
        try:
            ...
            if value[0] in CFWS_LEADER:
                ...
                if not value or value[0] == ",":
                    ...
                    address_list.defects.append(
                        errors.InvalidHeaderDefect("invalid address in address-list")
                    )
            elif value[0] == ",":
                address_list.defects.append(
                    errors.ObsoleteHeaderDefect("empty element in address-list")
                )
            else:
                token, value = get_invalid_mailbox(value, ",")
                if leader is not None:
...
def get_msg_id(value):
    ...
    try:
        ...
        try:
            ...
            msg_id.defects.append(
                errors.ObsoleteHeaderDefect("obsolete id-left in msg-id")
            )
        except errors.HeaderParseError:
            raise errors.HeaderParseError(
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                msg_id.defects.append(
                    errors.ObsoleteHeaderDefect("obsolete id-right in msg-id")
                )
            except errors.HeaderParseError:
                raise errors.HeaderParseError(
    ...
    if value and value[0] == ">":
        ...
        msg_id.defects.append(
            errors.InvalidHeaderDefect("missing trailing '>' on msg-id")
        )
    msg_id.append(ValueTerminal(">", "msg-id-end"))
    if value and value[0] in CFWS_LEADER:
        token, value = get_cfws(value)
...
def parse_mime_version(value):
    ...
    if not value:
        mime_version.defects.append(
            errors.HeaderMissingRequiredValue("Missing MIME version number (eg: 1.0)")
        )
        return mime_version
    if value[0] in CFWS_LEADER:
        token, value = get_cfws(value)
    ...
    if value:
        mime_version.defects.append(
            errors.InvalidHeaderDefect("Excess non-CFWS text after MIME version")
        )
        mime_version.append(ValueTerminal(value, "xtext"))
    return mime_version
...
def get_section(value):
    ...
    if digits[0] == "0" and digits != "0":
        section.defects.append(
            errors.InvalidHeaderDefect("section number has an invalid leading 0")
        )
...
def get_parameter(value):
    ...
    if not value:
        ...
        param.defects.append(
            errors.InvalidHeaderDefect("Missing required charset/lang delimiters")
        )
        appendto.append(token)
        ...
        try:
            ...
            if value[0] == ";":
                ...
                mime_parameters.defects.append(
                    errors.InvalidHeaderDefect("parameter entry with no content")
                )
            else:
...
def parse_content_disposition_header(value):
    ...
    if not value:
        disp_header.defects.append(
            errors.HeaderMissingRequiredValue("Missing content disposition")
        )
        return disp_header
    try:
        token, value = get_token(value)
...
def parse_content_transfer_encoding_header(value):
    ...
    if not value:
        cte_header.defects.append(
            errors.HeaderMissingRequiredValue("Missing content transfer encoding")
        )
        return cte_header
    try:
        token, value = get_token(value)
    ...
    while value:
        cte_header.defects.append(
            errors.InvalidHeaderDefect("Extra text after content transfer encoding")
        )
        if value[0] in PHRASE_ENDS:
...
def _refold_parse_tree(parse_tree, *, policy):
    ...
    while parts:
        ...
        try:
            ...
            if any(
                isinstance(x, errors.UndecodableBytesDefect) for x in part.all_defects
            ):
        ...
        if want_encoding and not wrap_as_ew_blocked:
            ...
            if not hasattr(part, "encode"):
                ...
                if (
                    last_ew is not None
                    and charset != last_charset
                    and (
                        last_charset == "unknown-8bit"
                        or last_charset == "utf-8"
...
def _fold_as_ew(
    to_encode, lines, maxlen, last_ew, ew_combine_allowed, charset, leading_whitespace
):
